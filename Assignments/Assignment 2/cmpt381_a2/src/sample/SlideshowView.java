...
public class SlideshowView extends VBox
{
    ...
    public SlideshowView(DataModel dataModel) throws FileNotFoundException{
    ...
    public void start() throws FileNotFoundException {
        this.getChildren().clear();
         ArrayList<String> imagesInSlideshow=dataModel.getSlideshowHashMap().get(dataModel.getSelectedSlideshow());
        if(imagesInSlideshow!=null){
            imgNames=new String[imagesInSlideshow.size()];
            int i=0;
            for(String img_url:imagesInSlideshow){
                imgNames[i]=img_url;
                i++;
            }
        }
        System.out.println(Arrays.toString(imgNames));
        initialise();
        setActions();
    ...
    private void initialise() throws FileNotFoundException {
                ...
                for (int i=0;i<imgNames.length;i++){
                    imgsList[i]=new Image(new FileInputStream(imgNames[i]));
                }
                ...
                switchBtn= new Button("Manual Navigation Mode");
                 backToSetup=new Button("Back");

                toolBox.getChildren().addAll(paurseOrResumeBtn,switchBtn,backToSetup);
                ...
                this.getChildren().add(slideShowVBox);
            ...
            private void setActions(){
                ...
                backToSetup.setOnAction(new EventHandler<ActionEvent>() {
                    @Override
                    public void handle(ActionEvent actionEvent) {
                        try {
                            pauseTimer();
                            dataModel.updateMainView(DataModel.SETUP_MODE);
